...
export default function DashboardScreen({ meals, announcements, upcomingExam, studySessions, schedules }: DashboardScreenProps) {
  ...
  return (
    <div className="relative min-h-dvh overflow-x-hidden">
      {/* Background */}
      <div className="absolute inset-0 bg-gradient-to-b from-emerald-50/50 via-white to-gray-50/50 pointer-events-none z-0" />
      ...
      <div className="relative z-10 flex flex-col">
        ...
        <header className="px-6 pt-12 pb-4 flex justify-between items-start">
          ...
          <Link
            href="/profil"
            className="bg-white p-2.5 rounded-xl shadow-sm border border-gray-100 relative group transition-transform active:scale-95"
          >
        ...
        <div className="flex-1 px-6 space-y-5">
            ...
            {nextPrayer ? (
              <div className="flex items-center justify-between relative z-10">
                ...
                <div className="text-right">
                  ...
                  <p className="text-xl font-bold text-gray-800 tabular-nums">{nextPrayer.countdown}</p>
                </div>
              </div>
            ) : prayerLoading ? (
              <p className="text-gray-400 text-sm">Yükleniyor...</p>
            ) : (
              <p className="text-gray-400 text-sm">Namaz vakitleri yüklenmedi</p>
            )}
              ...
              <div className="mt-4 flex justify-between gap-1.5 overflow-x-auto no-scrollbar pt-3 border-t border-gray-50">
                {PRAYER_KEYS.map((p) => {
                  ...
                  return (
                    ...
                    >
                      <span className={`text-[10px] mb-0.5 ${isNext ? "opacity-80" : "text-gray-500"}`}>{p.label}</span>
                      <span className={`text-sm font-bold ${isNext ? "" : "text-gray-800"}`}>
          ...
          <section className="grid grid-cols-4 gap-3 pb-10">
            <Link href="/ibadet" className="flex flex-col items-center gap-2 group">
              ...
              <span className="text-[10px] font-semibold text-gray-600 text-center">Namaz</span>
            </Link>
            <Link href="/ibadet" className="flex flex-col items-center gap-2 group">
              <div className="w-14 h-14 bg-white rounded-2xl flex items-center justify-center shadow-sm border border-gray-100 group-active:scale-95 transition-transform group-hover:border-primary/30 group-hover:shadow-md">
                <span className="material-icons-round text-primary text-2xl">menu_book</span>
              </div>
              <span className="text-[10px] font-semibold text-gray-600 text-center">Kur'an</span>
            </Link>
            <Link href="/takip" className="flex flex-col items-center gap-2 group">
              ...
              <span className="text-[10px] font-semibold text-gray-600 text-center">Denemeler</span>
            </Link>
            <button onClick={() => signOut()} className="flex flex-col items-center gap-2 group">
              <div className="w-14 h-14 bg-white rounded-2xl flex items-center justify-center shadow-sm border border-gray-100 group-active:scale-95 transition-transform group-hover:border-red-200 group-hover:shadow-md">
                <span className="material-icons-round text-gray-400 text-2xl">logout</span>
              </div>
              <span className="text-[10px] font-semibold text-gray-600 text-center">Çıkış</span>
            </button>
          </section>
        </div>
      </div>
