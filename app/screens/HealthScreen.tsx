    ...
    <div className="relative min-h-dvh overflow-x-hidden">
      ...
      <div className="relative z-10 flex flex-col">
        ...
        <section className="px-6 mb-6">
          <div className="bg-white rounded-2xl p-6 shadow-lg shadow-primary/5 border-l-4 border-primary relative overflow-hidden">
            ...
            <div className="relative z-10">
              ...
              <p className="text-gray-600 mb-4 leading-relaxed pr-8">
        ...
        <section className="px-6 mb-8">
          ...
          <div className="grid grid-cols-2 gap-4">
            <button className="group relative flex flex-col items-center justify-center p-6 bg-white rounded-2xl shadow-sm border border-gray-100 hover:border-primary/30 hover:shadow-md transition-all duration-300 overflow-hidden">
              <div className="absolute -right-4 -top-4 w-12 h-12 text-primary/10 group-hover:text-primary/20 transition-colors">
                <svg className="w-full h-full" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M12 2C11.45 2 11 2.45 11 3V4C7.69 4 5 6.69 5 10V17H7L8 20H16L17 17H19V10C19 6.69 16.31 4 13 4V3C13 2.45 12.55 2 12 2M12 22C11.45 22 11 21.55 11 21H13C13 21.55 12.55 22 12 22Z" />
                </svg>
              </div>
              <div className="w-14 h-14 rounded-full bg-red-50 flex items-center justify-center mb-3 group-hover:scale-110 transition-transform relative z-10">
            ...
            <button className="group relative flex flex-col items-center justify-center p-6 bg-white rounded-2xl shadow-sm border border-gray-100 hover:border-primary/30 hover:shadow-md transition-all duration-300 overflow-hidden">
              ...
              <div className="w-14 h-14 rounded-full bg-blue-50 flex items-center justify-center mb-3 group-hover:scale-110 transition-transform relative z-10">
        ...
        <section className="px-6 mb-6">
          <div className="flex justify-between items-end mb-4">
            <h3 className="text-lg font-semibold text-gray-800">Son 3 Günün Özeti</h3>
                  ...
                  return (
                    <div key={item.day} className="flex flex-col items-center gap-2 w-1/3 group">
                      <div className={`relative w-full bg-gray-50 rounded-t-lg h-full flex items-end overflow-hidden ${isToday ? "ring-2 ring-primary/20" : ""}`}>
                        ...
                        >
                          <span className={`absolute -top-6 left-1/2 -translate-x-1/2 text-xs font-bold ${isToday ? "text-primary" : "text-gray-600"}`}>
                            {item.value}
                          </span>
                        </div>
                      </div>
                      <span className={`text-xs font-medium ${isToday ? "text-primary font-bold" : "text-gray-500"}`}>
                        {item.day}
                      </span>
                    </div>
                  );
                ...
                {MOCK_PRESSURE.map((item, idx) => {
                  ...
                  return (
                    <div key={item.day} className={`flex items-center justify-between p-3 rounded-xl ${isToday ? "bg-primary/5 border border-primary/20" : "bg-gray-50"}`}>
                      <span className={`text-sm font-medium ${isToday ? "text-primary font-bold" : "text-gray-500"}`}>
                        {item.day}
                      </span>
                      <span className={`text-lg font-bold ${isToday ? "text-primary" : "text-gray-800"}`}>
                        {item.value}
                      </span>
        ...
        <section className="px-6 mb-8">
          <div className="bg-mint-soft/30 rounded-xl p-4 flex gap-4 items-center border border-primary/20 relative overflow-hidden">
            ...
            <div className="w-12 h-12 rounded-full bg-white flex items-center justify-center text-primary flex-shrink-0 shadow-sm border border-primary/10 relative z-10">
